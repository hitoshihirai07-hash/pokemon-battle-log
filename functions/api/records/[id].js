...
function normalise(body) {
  return {
    ...
    opponentPokemon: body.opponentPokemon.map(p => ({ baseName: String(p?.baseName || '').trim(), selected: Boolean(p?.selected), megaName: String(p?.megaName || '').trim(), moves: Array.isArray(p?.moves) ? p.moves.map(v => String(v || '').trim()).filter(Boolean).slice(0, 4) : [], item: String(p?.item || '').trim(), ability: String(p?.ability || '').trim(), note: String(p?.note || '').trim() })),
...
export async function onRequestPut({ request, env, params }) {
  ...
  await env.DB.prepare(`UPDATE records SET battle_date = ?, result = ?, team_id = ?, self_pokemon_json = ?, opponent_pokemon_json = ?, speed_json = ?, damage_json = ?, note = ?, other_note = ?, updated_at = datetime('now') WHERE id = ?`)
